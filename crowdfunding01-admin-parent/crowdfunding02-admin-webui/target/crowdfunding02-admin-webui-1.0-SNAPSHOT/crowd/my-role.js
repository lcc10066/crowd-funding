...
// 声明专门的函数 用来 在分配auth的模态框中显示树形结构的数据
function fullauthtree() {
    ...
    if (ajaxReturn.status != 200) {
        layer.msg("请求出错！响应状态码是" + ajaxReturn.status + "说明是："
...
function fillTableBody(pageInfo) {
    ...
    // 清除tbody中的旧的内容,不清除的话新增的内容会叠加到原有内容后面
    $("#rolePageBody").empty();
    ...
    for (var i = 0; i < pageInfo.list.length; i++) {
        ...
        // 为实现编辑操作：通过button标签的id属性（别的属性其实也可以）把roleId值传递到button按钮的单击响应函数中，在单击响应函数中使用this.id获取这个id值
...
function paginationCallBack(pageIndex, jQuery) {
    ...
    // 调用分页函数，不会造成递归调用，只有在点击获取其他页时，该回调函数才会执行
    generatePage();
    // 取消页码超链接的默认行为
    return false;
}